;(function (dice) {
  ...
  const create_d4_materials = (size, margin, labels) => {
    const create_d4_text = (text, color, back_color) => {
      ...
      canvas.width = canvas.height = ts
      context.font = (ts - margin) / 2.2 + 'pt Arial'
      context.fillStyle = back_color
      context.fillRect(0, 0, canvas.width, canvas.height)
      context.textAlign = 'center'
      context.textBaseline = 'middle'
  ...
  this.dice_box.prototype.reinit = function (container) {
    ...
    let wh = ch / aspect / Math.tan((10 * Math.PI) / 200)
  ...
  this.dice_box.prototype.roll = function (vectors, values, callback) {
    ...
    if (values != undefined && values.length) {
      ...
      let res = emulate_throw()
      prepare_dices_for_roll(vectors)
      for (let i in res) shift_dice_faces(dices[i], values[i], res[i])
    }
  ...
  this.dice_box.prototype.bind_mouse = function (
    ...
    bind(container, ['mousedown', 'touchstart'], function (ev) {
      ...
      mouse_start = get_mouse_coords(ev)
    })
    ...
    bind(container, ['mouseup', 'touchend'], function (ev) {
      ...
      var time_int = new Date().getTime() - mouse_time
