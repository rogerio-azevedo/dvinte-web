...
interface MessageProps extends FromProps {
  $crit?: 'HIT' | 'FAIL' | null
}
...
export const MessageData = styled.div<FromProps>`
  margin-bottom: 15px;
...
export const MessageDataName = styled.span<FromProps>`
  color: #000;
  padding-left: 6px;
  font-size: 14px;
...
export const MessageContainer = styled.div<FromProps>`
  display: flex;
  flex-direction: column;
...
export const ListMessage = styled.li<FromProps>`
  list-style: none;
  text-align: ${props => (props.$from ? 'right' : 'left')};
