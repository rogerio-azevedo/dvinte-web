/* eslint-disable no-console */
import React, { useRef, useEffect, useState } from 'react'
import { useSelector } from 'react-redux'

import * as THREE from 'three'
import * as CANNON from 'cannon'

import { Container } from './styles'

import {
  DiceManager,
  DiceD4,
  DiceD6,
  DiceD8,
  DiceD10,
  DiceD12,
  DiceD20,
} from './dice'

// Import DiceObject type
import type { DiceObject } from './dice'

interface DicesState {
  diceType: string
  diceSides: number
  diceMult: number
  diceResult: number[]
  diceRoll: boolean
}

interface RootState {
  dices: DicesState
}

interface Controls {
  start: () => void
  stop: () => void
}

type DiceType = 'd4' | 'd6' | 'd8' | 'd10' | 'd12' | 'd20'

const MyDices: React.FC = () => {
  const { diceType, diceSides, diceMult, diceResult, diceRoll } = useSelector(
    (state: RootState) => state.dices
  )

  const [roll, setRoll] = useState<boolean>(diceRoll)
  const mount = useRef<HTMLDivElement>(null)
  const controls = useRef<Controls | null>(null)

  let diceT: DiceObject | null = null

  const width = window.innerWidth * 0.8
  const height = window.innerHeight * 0.8
  let frameId: number | null = null

  const dice: DiceObject[] = []
  let scene: THREE.Scene
  let camera: THREE.PerspectiveCamera
  let renderer: THREE.WebGLRenderer
  let world: CANNON.World
  let barrier: CANNON.Body

  const dice_color = '#200122'
  const ambient_light_color = 0xf0f5fb
  const spot_light_color = 0xefdfd5

  const dice_box = (): (() => void) => {
    scene = new THREE.Scene()
    world = new CANNON.World()

    renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true })
    renderer.shadowMap.enabled = true
    renderer.shadowMap.type = THREE.PCFShadowMap
    renderer.setSize(width, height)
    renderer.setClearColor(0xffffff, 0)

    camera = new THREE.PerspectiveCamera(18, width / height, 1, 10000)
    camera.position.set(10, 80, 40)
    camera.lookAt(new THREE.Vector3(10, 40, 40))
    camera.position.z = 1
    camera.position.x = -1

    const ambient = new THREE.AmbientLight(ambient_light_color, 0.8)
    scene.add(ambient)

    const directionalLight = new THREE.DirectionalLight(spot_light_color, 1.2)
    directionalLight.position.x = -1000
    directionalLight.position.y = 1000
    directionalLight.position.z = 1000
    scene.add(directionalLight)

    const light = new THREE.SpotLight(spot_light_color, 0.4)
    light.position.y = 100
    light.target.position.set(0, 0, 0)
    light.castShadow = true
    light.shadow.camera.near = 50
    light.shadow.camera.far = 110
    light.shadow.mapSize.width = 1024
    light.shadow.mapSize.height = 1024
    scene.add(light)

    // FLOOR
    const floorMaterial = new THREE.MeshPhongMaterial({
      // color: '#00aa00',
      // side: THREE.DoubleSide,
    })
    const floorGeometry = new THREE.PlaneGeometry(48, 25, 30)
    const floor = new THREE.Mesh(floorGeometry, floorMaterial)
    floor.receiveShadow = true
    floor.rotation.x = Math.PI / 2
    scene.add(floor)

    world.gravity.set(0, -9.82 * 20, 0)
    world.broadphase = new CANNON.NaiveBroadphase()
    world.solver.iterations = 16

    DiceManager.setWorld(world)

    //Floor
    const floorBody = new CANNON.Body({
      mass: 0,
      shape: new CANNON.Plane(),
      material: DiceManager.floorBodyMaterial,
    })

    floorBody.quaternion.setFromAxisAngle(
      new CANNON.Vec3(1, 0, 0),
      -Math.PI / 2
    )
    world.add(floorBody)

    //Walls
    barrier = new CANNON.Body({
      mass: 0,
      shape: new CANNON.Plane(),
      material: DiceManager.barrierBodyMaterial,
    })
    barrier.quaternion.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), -Math.PI / 2)
    barrier.position.set(500 * 0.93, 0, 0)
    world.add(barrier)

    //PAREDE DIREITA
    barrier = new CANNON.Body({
      mass: 0,
      shape: new CANNON.Plane(),
      material: DiceManager.barrierBodyMaterial,
    })
    barrier.quaternion.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), -Math.PI / 2)
    barrier.position.set(1, width, 0)
    world.add(barrier)

    for (let i = 0; i < diceMult; i++) {
      switch (diceType as DiceType) {
        case 'd4':
          diceT = new DiceD4({ size: 1.4, backColor: dice_color })
          break
        case 'd6':
          diceT = new DiceD6({ size: 1.4, backColor: dice_color })
          break
        case 'd8':
          diceT = new DiceD8({ size: 1.4, backColor: dice_color })
          break
        case 'd10':
          diceT = new DiceD10({ size: 1.4, backColor: dice_color })
          break
        case 'd12':
          diceT = new DiceD12({ size: 1.4, backColor: dice_color })
          break
        case 'd20':
          diceT = new DiceD20({ size: 1.4, backColor: dice_color })
          break
        default:
          diceT = new DiceD6({ size: 1.4, backColor: dice_color })
          break
      }

      if (diceT) {
        const dieObject = diceT.getObject()
        if (dieObject) {
          scene.add(dieObject)
          dice.push(diceT)
        }
      }
    }

    const renderScene = (): void => {
      renderer.render(scene, camera)
    }

    const handleResize = (): void => {
      if (!mount.current) return

      const newWidth = mount.current.clientWidth
      const newHeight = mount.current.clientHeight
      renderer.setSize(newWidth, newHeight)
      camera.aspect = newWidth / newHeight
      camera.updateProjectionMatrix()
      renderScene()
    }

    const animate = (): void => {
      renderScene()
      updatePhysics()
      update()
      frameId = window.requestAnimationFrame(animate)
    }

    function updatePhysics(): void {
      world.step(1.0 / 60.0)

      for (const die of dice) {
        die.updateMeshFromBody()
      }
    }

    function update(): void {
      if (controls.current) {
        controls.current.stop()
      }
    }

    const start = (): void => {
      if (!frameId) {
        frameId = requestAnimationFrame(animate)
      }
    }

    const stop = (): void => {
      if (frameId) {
        cancelAnimationFrame(frameId)
        frameId = null
      }
    }

    if (mount.current) {
      mount.current.appendChild(renderer.domElement)
    }

    window.addEventListener('resize', handleResize)
    start()

    controls.current = { start, stop }

    return (): void => {
      stop()
      window.removeEventListener('resize', handleResize)

      if (mount.current && renderer.domElement.parentNode === mount.current) {
        mount.current.removeChild(renderer.domElement)
      }

      // Clean up Three.js resources
      for (const die of dice) {
        const dieObject = die.getObject()
        if (dieObject) {
          scene.remove(dieObject)
          if (dieObject.geometry) {
            dieObject.geometry.dispose()
          }
          if (Array.isArray(dieObject.material)) {
            dieObject.material.forEach(material => material.dispose())
          } else if (dieObject.material) {
            dieObject.material.dispose()
          }
        }
      }

      renderer.dispose()
    }
  }

  function randomDiceThrow(): void {
    const diceValues: Array<{ dice: DiceObject; value: number }> = []

    for (let i = 0; i < dice.length; i++) {
      const yRand = Math.floor(Math.random() * diceSides) + 1
      const dieObject = dice[i].getObject()

      if (dieObject) {
        dieObject.position.x = -35 - (i % 3) * 1.5
        dieObject.position.y = 2 + Math.floor(i / 3) * 1.5
        dieObject.position.z = -15 + (i % 3) * 1.5
        dieObject.quaternion.x = ((Math.random() * 90 - 45) * Math.PI) / 180
        dieObject.quaternion.z = ((Math.random() * 90 - 45) * Math.PI) / 180

        dice[i].updateBodyFromMesh()

        const rand = Math.random() * 5
        const body = dieObject.body

        if (body) {
          body.velocity.set(45 + rand, 40 + yRand, 15 + rand)
          body.angularVelocity.set(
            30 * Math.random() - 10,
            30 * Math.random() - 10,
            30 * Math.random() - 10
          )
        }

        diceValues.push({
          dice: dice[i],
          value: diceResult[i] || 1,
        })
      }
    }

    try {
      DiceManager.prepareValues(diceValues)
    } catch (error) {
      console.error('Erro ao preparar valores dos dados:', error)
    }
  }

  useEffect(() => {
    setRoll(false)
    const cleanup = dice_box()

    return cleanup
  }, []) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (diceRoll && !DiceManager.throwRunning) {
      randomDiceThrow()
    }
  }, [diceRoll]) // eslint-disable-line react-hooks/exhaustive-deps

  return <Container ref={mount} />
}

export default MyDices
